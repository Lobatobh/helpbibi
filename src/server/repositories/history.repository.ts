...
const mapListItem = (s: any, viewerRole: 'CLIENT' | 'PROVIDER'): HistoryListItem => {
  const base: HistoryListItem = {
    ...
    paymentStatus: s.paymentStatus || 'PENDING', createdAt: s.createdAt.toISOString(),
...
export async function getClientServices(actor: HistoryActor, limit: number = 50): Promise<HistoryListItem[]> {
  ...
  const services = await db.serviceRequest.findMany({ where: { clientId: actor.userId }, include: { provider: { include: { user: true } }, client: true, ratings: true }, orderBy: { createdAt: 'desc' }, take: limit })
...
export async function getClientServiceDetail(actor: HistoryActor, serviceId: string): Promise<{ status: number; data: HistoryDetail | { message: string } }> {
  ...
  const svc = await db.serviceRequest.findUnique({ where: { id: serviceId }, include: { provider: { include: { user: true } }, client: true, ratings: true, timeline: { orderBy: { createdAt: 'asc' } } } })
...
export async function getProviderServices(actor: HistoryActor, actorProviderProfileId: string | null, limit: number = 50): Promise<HistoryListItem[]> {
  ...
  const services = await db.serviceRequest.findMany({ where: { providerId: actorProviderProfileId }, include: { provider: { include: { user: true } }, client: true, ratings: true }, orderBy: { createdAt: 'desc' }, take: limit })
